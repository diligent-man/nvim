...
local function autoclose_tab_buffer(winnr)
    ...
    local tabnr = win_get_tabpage(winnr)

    local bufnr = win_get_buf(winnr)
    local buf_info = getbufinfo(bufnr)[1]

    local tab_wins = filter(function(w) return w ~= winnr end, tabpage_list_wins(tabnr))
    local tab_bufs = map(win_get_buf, tab_wins)

    -- close buffer was nvim tree
    if buf_info.name:match(".*NvimTree_%d*$") then
        -- Close all nvim tree on :q
        if not isempty(tab_bufs) then                             -- and was not the last window (not closed automatically by code below)
            api.tree.close()
        end
        ...
        if #tab_bufs == 1 then                                    -- if there is only 1 buffer left in the tab
            ...
            if last_buf_info.name:match(".*NvimTree_%d*$") then   -- and that buffer is nvim tree
                schedule(function ()
                    if #list_wins() == 1 then                -- if its the last buffer in vim
                        vicmd("quit")                             -- then close all of vim
                    else                                          -- else there are more tabs open
                        win_close(tab_wins[1], true)         -- then close only the tab
                    end
                end)
            end
        end
